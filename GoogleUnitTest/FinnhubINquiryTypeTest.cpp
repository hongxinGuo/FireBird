...
namespace FireBirdTest {
	TEST(FinnhubInquiryTypeTest, Test) {
		...
		EXPECT_EQ(SYMBOL_LOOKUP_, WEBSOCKET_NEWS_ + 1);
		...
		EXPECT_EQ(STOCK_ESTIMATES_RECOMMENDATION_TRENDS_, ISIN_CHANGE_ + 1);
		...
		EXPECT_EQ(STOCK_PRICE_QUOTE_, STOCK_ESTIMATES_EARNING_CALENDAR_ + 1);
		...
		EXPECT_EQ(ETFS_INDICES_CONSTITUENTS_, STOCK_PRICE_DIVIDEND2_ + 1);
		...
		EXPECT_EQ(MUTUAL_FUND_PROFILE_, ETFS_COUNTRY_ + 1);
		...
		EXPECT_EQ(MUTUAL_FUNDS_EET_, MUTUAL_FUND_COUNTRY_ + 1);
		EXPECT_EQ(MUTUAL_FUNDS_EET_PAI_, MUTUAL_FUNDS_EET_ + 1);

		EXPECT_EQ(BOND_PROFILE_, MUTUAL_FUNDS_EET_PAI_ + 1);
		EXPECT_EQ(BOND_PRICE_, BOND_PROFILE_ + 1);
		EXPECT_EQ(BOND_TICK_TRADE_DATA_, BOND_PRICE_ + 1);
		EXPECT_EQ(BOND_YIELD_CURVE_, BOND_TICK_TRADE_DATA_ + 1);

		EXPECT_EQ(FOREX_EXCHANGE_, BOND_YIELD_CURVE_ + 1);
		...
		EXPECT_EQ(CRYPTO_EXCHANGE_, FOREX_ALL_RATES_ + 1);
		...
		EXPECT_EQ(TECHNICAL_ANALYSIS_PATTERN_RECOGNITION_, CRYPTO_CANDLES_ + 1);
		...
		EXPECT_EQ(ALTERNATIVE_DATA_TRANSCRIPT_LIST_, TECHNICAL_ANALYSIS_TECHNICAL_INDICATORS_ + 1);
		...
		EXPECT_EQ(ECONOMIC_COUNTRY_LIST_, ALTERNATIVE_DATA_FDA_CALENDAR_ + 1);
		...
		// Tiingo specific
		EXPECT_EQ(TIINGO_FUNDAMENTAL_DEFINITION_, ECONOMIC_ECONOMIC_ + 1);
		EXPECT_EQ(TIINGO_STOCK_DAILY_META_, TIINGO_FUNDAMENTAL_DEFINITION_ + 1);
		EXPECT_EQ(TIINGO_FINANCIAL_STATEMENT_, TIINGO_STOCK_DAILY_META_ + 1);
		EXPECT_EQ(TIINGO_IEX_TOP_OF_BOOK_, TIINGO_FINANCIAL_STATEMENT_ + 1);

		// Accessory product
		EXPECT_EQ(ACCESSORY_INDEX_NASDAQ100_STOCKS_, TIINGO_IEX_TOP_OF_BOOK_ + 1);
	}
}
