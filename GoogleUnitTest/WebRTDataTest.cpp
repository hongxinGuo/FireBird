...
namespace StockAnalysisTest {
	...
	NeteaseRTSymbolIndex neteaseData3(3, _T("name"), 3, _T("招商轮船"));
	...
	TEST_P(NeteaseRTDataTest, TestGetNeteaseSymbolIndex) {
		...
		case 2: // code
			m_RTData.SetNeteaseRTValue(lIndex, m_strValue);
			EXPECT_STREQ(m_RTData.GetSymbol(), _T("601872.SS"));
			break;
		case 3: // name
			m_RTData.SetNeteaseRTValue(lIndex, m_strValue);
	...
	TEST_F(CStockWebRTDataTest, TestSaveData) {
		...
		time_t tt = TransferToTTime(21101123, -8 * 3600, 150000);
	...
	TEST_F(CStockWebRTDataTest, TestLoadData) {
		...
		EXPECT_STREQ(id.GetSymbol(), pRTData->GetSymbol());
