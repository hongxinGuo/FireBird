...
namespace FireBirdTest {
	...
	TEST_F(CContainerTiingoStockDayLineTest, TestUpdateDB2_InsertsAndUpdates) {
		using namespace StockMarket;
		const auto& t = TiingoStockDayline{};

		const std::string testSymbol = "UNITTEST.TS";

		// Ensure DB is clean for the test symbol
		{
			auto db = gl_dbStockMarket.get();
			auto tx = sqlpp::start_transaction(db);
		...
		// Prepare a container with two day lines
		CContainerTiingoStockDayLine container;
		{
			CTiingoCandleLine c1;
			c1.Reset();
			c1.SetDate(20230101);
			c1.SetExchange("NYSE");
			c1.SetStockSymbol(testSymbol);
			c1.SetLastClose(100.0);
			c1.SetOpen(101.0);
			c1.SetHigh(103.0);
			c1.SetLow(99.0);
			c1.SetClose(102.0);
			c1.SetSplitFactor(1.0);
			c1.SetDividend(0.0);
			c1.SetUpDown(2.0);
			c1.SetVolume(1000);
			c1.SetAmount(102000);
			c1.SetUpDownRate(2.0);
			c1.SetChangeHandRate(0.5);
			c1.SetTotalValue(1.0e9);
			c1.SetCurrentValue(5.0e8);
			container.Add(c1);

			CTiingoCandleLine c2;
			c2.Reset();
			c2.SetDate(20230102);
			c2.SetExchange("NYSE");
			c2.SetStockSymbol(testSymbol);
			c2.SetLastClose(102.0);
			c2.SetOpen(102.5);
			c2.SetHigh(104.0);
			c2.SetLow(101.0);
			c2.SetClose(103.0);
			c2.SetSplitFactor(1.0);
			c2.SetDividend(0.0);
			c2.SetUpDown(1.0);
			c2.SetVolume(1100);
			c2.SetAmount(113300);
			c2.SetUpDownRate(0.98);
			c2.SetChangeHandRate(0.4);
			c2.SetTotalValue(1.01e9);
			c2.SetCurrentValue(5.1e8);
			container.Add(c2);
		}
		...
		// Verify DB contains two rows for the symbol
		{
			auto db = gl_dbStockMarket.get();
			auto tx = sqlpp::start_transaction(db);
			auto result = db(select(all_of(t)).from(t).where(t.Symbol == testSymbol).order_by(t.Date.asc()));
			tx.commit();
			EXPECT_EQ(result.size(), 2u) << "After first UpdateDB2, DB should contain two rows for the test symbol";
		}

		// Add an earlier date and call UpdateDB2 again -> should insert the missing earlier row
		{
			CTiingoCandleLine c0;
			c0.Reset();
			c0.SetDate(20221231);
			c0.SetExchange("NYSE");
			c0.SetStockSymbol(testSymbol);
			c0.SetLastClose(98.0);
			c0.SetOpen(98.5);
			c0.SetHigh(100.0);
			c0.SetLow(97.0);
			c0.SetClose(99.0);
			c0.SetSplitFactor(1.0);
			c0.SetDividend(0.0);
			c0.SetUpDown(-1.0);
			c0.SetVolume(900);
			c0.SetAmount(89100);
			c0.SetUpDownRate(-1.0);
			c0.SetChangeHandRate(0.3);
			c0.SetTotalValue(9.8e8);
			...
			auto result = db(select(all_of(t)).from(t).where(t.Symbol == testSymbol).order_by(t.Date.asc()));
			tx.commit();
			EXPECT_EQ(result.size(), 3u) << "After inserting an earlier date, DB should contain three rows for the test symbol";
		}
