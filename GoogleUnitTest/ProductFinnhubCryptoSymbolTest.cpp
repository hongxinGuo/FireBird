...
namespace FireBirdTest {
	...
	TEST_F(CFinnhubCryptoSymbolProductTest, TestCreatMessage) {
		...
		EXPECT_STREQ(cryptoSymbolProduct.CreateMessage().c_str(), (cryptoSymbolProduct.GetInquiryFunction() + gl_dataContainerFinnhubCryptoExchange.GetItem(1)).c_str());
	}
	...
	protected:
		void SetUp() override {
			...
			m_finnhubCryptoSymbolProduct.__Test_checkAccessRight(m_pWebData);
