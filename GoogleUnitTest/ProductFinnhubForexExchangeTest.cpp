...
namespace FireBirdTest {
	...
	TEST_F(CFinnhubForexExchangeTest, TestUpdateDataSourceStatus) {
		...
		forexExchange.UpdateSystemStatus();
	...
	namespace {
		...
		Test_FinnhubWebData finnhubWebData72(2, "", R"("oanda","fxcm","forex.com","pepperstone","deriv","icmtrader","ic markets","fxpig"])");
		// 格式不对
		Test_FinnhubWebData finnhubWebData73(3, "", R"(["oanda",fxcm,"forex.com","pepperstone","deriv","icmtrader","ic markets","fxpig"])");
		// 空数据
		...
		// 正确的数据
		Test_FinnhubWebData finnhubWebData80(10, "", R"(["new exchange","fxcm","forex.com","pepperstone","deriv","icmtrader","ic markets","fxpig"])");
	}
	...
	protected:
		void SetUp() override {
			...
			m_text = pData->m_data;
			m_finnhubForexExchange.Test_checkAccessRight_(m_text);
	...
	TEST_P(ParseFinnhubForexExchangeTest, TestParseFinnhubForexExchange0) {
		m_pvExchange = m_finnhubForexExchange.Parse(m_text);
		switch (m_index) {
		case 2: // 格式不对
