...
namespace FireBirdTest {
	...
	TEST_F(CMockQuandlDataSourceTest, TestGenerateInquiryMessage1) {
		...
		EXPECT_FALSE(s_pMockQuandlDataSource->GenerateInquiryMessage(toLocalTime(120500))) << "时间未到，继续等待";
		EXPECT_FALSE(s_pMockQuandlDataSource->IsInquiring());
		EXPECT_FALSE(s_pMockQuandlDataSource->HaveInquiry());
		EXPECT_TRUE(s_pMockQuandlDataSource->GenerateInquiryMessage(toLocalTime(120500))) << "申请数据";
