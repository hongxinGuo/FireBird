...
namespace FireBirdTest {
	...
	void EnvironmentCheck() {
		...
		EXPECT_TRUE((GetUTCTime() + 3600) > tt) << GetUTCTime();
		EXPECT_EQ(s_tCurrentSystemTime, gl_tpNow);
	}
	...
	void SystemConfigurationCheck() {
		...
		EXPECT_EQ(gl_systemConfiguration.GetWorldMarketTiingoInquiryTime().count(), 500) << "默认每每次查询时间为500毫秒";
	...
	void ChinaMarketCheck() {
		if (gl_pChinaMarket != nullptr) {
			...
			EXPECT_EQ(gl_dataContainerChinaStock.Size(), 5701) << "测试数据库装载的股票数";
