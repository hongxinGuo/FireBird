...
namespace StockAnalysisTest {
	...
	TEST_F(CChinaStockTest, TestIsClearDayLineVector) {
		CChinaStock stock;
	...
	TEST_F(CChinaStockTest, TestIsSaveStock) {
		...
		pStock2 = gl_pChinaMarket->GetStock(2);
		EXPECT_FALSE(pStock->IsSameStock(pStock2));
		pStock2 = gl_pChinaMarket->GetStock(1);
	...
	TEST_F(CChinaStockTest, TestSaveStockCodeDB) {
		...
		EXPECT_EQ(setChinaStockSymbol.m_IPOStatus, __STOCK_IPOED__);
	...
	TEST_F(CChinaStockTest, TestUpdateStockCodeDB) {
		...
		setChinaStockSymbol.Open();
		EXPECT_THAT(setChinaStockSymbol.m_IPOStatus, Eq(__STOCK_NULL__)) << "此时状态已变为NULL";
		setChinaStockSymbol.Edit();
		setChinaStockSymbol.m_IPOStatus = __STOCK_DELISTED__;
	...
	TEST_F(CChinaStockTest, TestLoadStockCodeDB3) {
		...
		EXPECT_EQ(stock.GetIPOStatus(), setChinaStockSymbol.m_IPOStatus);
		EXPECT_EQ(stock.GetDayLineStartDate(), setChinaStockSymbol.m_DayLineStartDate);
		EXPECT_EQ(stock.GetDayLineEndDate(), gl_pChinaMarket->GetPrevDay(gl_pChinaMarket->GetFormatedMarketDate(), 31));
		setChinaStockSymbol.Close();
	}
	...
	TEST_F(CChinaStockTest, TestLoadDayLineAndDayLineInfo) {
		...
		CDayLinePtr pid;
		CDayLine stock;
		pStock = gl_pChinaMarket->GetStock(_T("600011.SS"));
		EXPECT_FALSE(gl_pChinaMarket->IsDayLineDBUpdated());
		gl_pChinaMarket->__TEST_SetFormatedMarketDate(21900101);
	...
	TEST_F(CChinaStockTest, TestSaveDayLine) {
		...
		CDayLinePtr pid;
		CDayLine dayLine;
		pStock = gl_pChinaMarket->GetStock(_T("600016.SS"));
		EXPECT_FALSE(gl_pChinaMarket->IsDayLineDBUpdated());
	...
	TEST_F(CChinaStockTest, TestUpdateDayLineStartEndDate) {
		...
		pStock = gl_pChinaMarket->GetStock(_T("600004.SS"));
	...
	TEST_F(CChinaStockTest, TestUpdateDayLineStartEndDate3) {
		...
		pStock = gl_pChinaMarket->GetStock(_T("600008.SS"));
