#include"pch.h"

#include"afxinet.h"

#include"MockInquireEngine.h"

#include"GeneralCheck.h"
#include "ProductDummy.h"
#include "WebData.h"

using namespace testing;

namespace {
	CMockInquireEnginePtr s_pInquireEngine; // 为了方便查找出错位置,将mock变量声明为全局的。
	CVirtualProductWebDataPtr s_pProduct;
}

namespace FireBirdTest {
	class CMockInquireEngineTest : public Test {
	protected:
		static void SetUpTestSuite() {
			SCOPED_TRACE("");
			GeneralCheck();
			s_pInquireEngine = nullptr;
		}

		static void TearDownTestSuite() {
			s_pInquireEngine = nullptr;
			SCOPED_TRACE("");
			GeneralCheck();
		}

		void SetUp() override {
			SCOPED_TRACE("");
			GeneralCheck();

			s_pInquireEngine = make_shared<CMockInquireEngine>();
			s_pProduct = make_shared<CProductDummy>();
		}

		void TearDown() override {
			// clearUp

			s_pInquireEngine = nullptr;
			SCOPED_TRACE("");
			GeneralCheck();
		}
	};

	TEST_F(CMockInquireEngineTest, TestGetWebData1) {
		EXPECT_FALSE(s_pInquireEngine->IsWebError());
		EXPECT_EQ(gl_systemMessage.ErrorMessageSize(), 0);

		EXPECT_CALL(*s_pInquireEngine, OpenFile).Times(1)
		.WillOnce([] {
			auto e = new CInternetException(1);
			throw e;
		});
		EXPECT_CALL(*s_pInquireEngine, GetFileHeaderInformation).Times(0);

		EXPECT_TRUE(s_pInquireEngine->GetWebData()->GetBufferLength() == 0) << "出现异常时数据为空";

		EXPECT_TRUE(s_pInquireEngine->IsWebError());
		EXPECT_EQ(gl_systemMessage.ErrorMessageSize(), 1) << "出现exception时报告错误信息";

		// 恢复原状
		gl_systemMessage.PopErrorMessage();
	}

	TEST_F(CMockInquireEngineTest, TestGetWebData2) {
		EXPECT_FALSE(s_pInquireEngine->IsWebError());
		EXPECT_EQ(gl_systemMessage.ErrorMessageSize(), 0);
		gl_systemConfiguration.SetExitingSystem(true); // 退出系统

		EXPECT_CALL(*s_pInquireEngine, OpenFile).Times(1);
		EXPECT_CALL(*s_pInquireEngine, GetFileHeaderInformation).Times(1)
		.WillOnce([] {
			s_pInquireEngine->SetContentLength(0);
		});
		EXPECT_CALL(*s_pInquireEngine, ReadWebFileOneTime).Times(0);

		EXPECT_TRUE(s_pInquireEngine->GetWebData()->GetBufferLength() == 0) << "出现异常时数据为空";

		EXPECT_EQ(s_pInquireEngine->GetBufferSize(), 1024 * 1024) << "不知道数据长度时，设置初始缓冲区为1M";

		EXPECT_TRUE(s_pInquireEngine->IsWebError());
		EXPECT_EQ(gl_systemMessage.ErrorMessageSize(), 1) << "系统退出时报告错误信息";

		// 恢复原状
		gl_systemConfiguration.SetExitingSystem(false);
		gl_systemMessage.PopErrorMessage();
	}

	TEST_F(CMockInquireEngineTest, TestGetWebData3) {
		EXPECT_FALSE(s_pInquireEngine->IsWebError());
		EXPECT_EQ(gl_systemMessage.ErrorMessageSize(), 0);

		EXPECT_CALL(*s_pInquireEngine, OpenFile).Times(1);
		EXPECT_CALL(*s_pInquireEngine, GetFileHeaderInformation).Times(1)
		.WillOnce([] {
			s_pInquireEngine->SetContentLength(2048);
		});
		EXPECT_CALL(*s_pInquireEngine, ReadWebFileOneTime).Times(0);
		EXPECT_CALL(*s_pInquireEngine, ReadWebFileOneTime).Times(2)
		.WillOnce(Return(1024))
		.WillOnce(Return(0));

		CWebDataPtr pWebData = s_pInquireEngine->GetWebData();
		EXPECT_TRUE(pWebData->GetBufferLength() > 0) << "正常结束时数据不为空";

		EXPECT_FALSE(s_pInquireEngine->IsWebError());
		EXPECT_EQ(s_pInquireEngine->GetBufferSize(), 0) << "生成WebData后，其内容std::move至WebData中，故而为零";
		EXPECT_EQ(s_pInquireEngine->GetByteRead(), 1024);

		EXPECT_EQ(pWebData->GetBufferLength(), 1024) << "数据移至此处";
		EXPECT_EQ(pWebData->GetTime(), gl_tpNow);

		EXPECT_EQ(gl_systemMessage.ErrorMessageSize(), 1) << "m_lContentLength != m_lByteRead, 系统报告数据长度不符错误";

		// 恢复原状
		gl_systemMessage.PopErrorMessage();
	}

	TEST_F(CMockInquireEngineTest, TestGetWebData4) {
		EXPECT_FALSE(s_pInquireEngine->IsWebError());
		EXPECT_EQ(gl_systemMessage.ErrorMessageSize(), 0);

		EXPECT_CALL(*s_pInquireEngine, OpenFile).Times(1);
		EXPECT_CALL(*s_pInquireEngine, GetFileHeaderInformation).Times(1)
		.WillOnce([] {
			s_pInquireEngine->SetContentLength(1024 * 17);
		});
		EXPECT_CALL(*s_pInquireEngine, ReadWebFileOneTime).Times(0);
		EXPECT_CALL(*s_pInquireEngine, ReadWebFileOneTime).Times(3)
		.WillOnce(Return(1024 * 16))
		.WillOnce(Return(1024))
		.WillOnce(Return(0));

		CWebDataPtr pWebData = s_pInquireEngine->GetWebData();
		EXPECT_TRUE(pWebData->GetBufferLength() > 0) << "正常结束时数据不为空";

		EXPECT_FALSE(s_pInquireEngine->IsWebError());
		EXPECT_EQ(s_pInquireEngine->GetBufferSize(), 0) << "生成WebData后，其内容std::move至WebData中，故而为零";
		EXPECT_EQ(s_pInquireEngine->GetByteRead(), 1024 * 17);

		EXPECT_EQ(pWebData->GetBufferLength(), 1024 * 17) << "数据移至此处";
		EXPECT_EQ(pWebData->GetTime(), gl_tpNow);

		EXPECT_EQ(gl_systemMessage.ErrorMessageSize(), 0) << "m_lContentLength == m_lByteRead";

		// 恢复原状
	}
}
