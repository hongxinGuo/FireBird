...
namespace FireBirdTest {
	...
	TEST_F(CFinnhubMarketHolidayProductTest, TestCreatMessage) {
		...
		EXPECT_EQ(MarketHolidayProduct.CreateMessage()->front(), (MarketHolidayProduct.GetInquiryFunction() + gl_dataContainerStockExchange.GetItemExchangeCode(1)));
	}
	...
	protected:
		void SetUp() override {
			...
			m_text = pData->m_data;
			m_finnhubMarketHolidayProduct.Test_checkAccessRight_(m_text);
	...
	TEST_P(ParseFinnhubMarketHolidayTest, TestParseFinnhubMarketHoliday0) {
		m_pvMarketHoliday = m_finnhubMarketHolidayProduct.Parse(m_text);
		switch (m_index) {
		case 0: // 空数据
