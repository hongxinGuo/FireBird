...
namespace FireBirdTest {
	...
	TEST_F(TimeConvertTest, TestTransferToDateTime) {
		...
		const INT64 lDateTime3 = ConvertToDateTime(tt, gl_pChinaMarket->GetMarketTimeZoneOffset().count()); // 东八区时间
	...
	TEST_F(TimeConvertTest, TestTransferToTTime) {
		if (gl_pWorldMarket->GetMarketTimeZoneOffset().count() == 4 * 3600) { // 美东夏时制？
			EXPECT_EQ(315601200, ConvertToTTime(19800101, gl_pWorldMarket->GetMarketTimeZoneOffset().count(), 150000)) << "美东标准时间的19800101150000，其UTC时间为315601200";
		}
		else {
			EXPECT_EQ(315604800, ConvertToTTime(19800101, gl_pWorldMarket->GetMarketTimeZoneOffset().count(), 150000)) << "美东夏时制标准时间的19800101150000，其UTC时间为315601200";
		}
		EXPECT_EQ(315558000, ConvertToTTime(19800101, gl_pChinaMarket->GetMarketTimeZoneOffset().count(), 150000)) << "北京标准时间的19800101150000，其UTC时间为315558000";
	}
