...
namespace FireBirdTest {
	...
	TEST_F(CDownLoadedNeteaseDayLineTest, TestSkipNeteaseDayLineFirstInformationLine) {
		...
		string str = "日期,股票代码,名称,收盘价,最高价,最低价,开盘价,前收盘,涨跌额,换手率,成交量,成交金额,总市值,流通市值\r\n";
		...
		str = "日期,股票代码,名称,收盘价,最高价,最低价,开盘价,前收盘,涨跌额,换手率,成交量,成交金额,总市值,流通市值\n"; // 缺少\r
		DayLine.SetStockCode("600000.SS");
		EXPECT_FALSE(DayLine.SkipNeteaseDayLineInformationHeader(str, lCurrentPos));
		EXPECT_EQ(lCurrentPos, str.size());
		lCurrentPos = 0;

		str = "日期,股票代码,名称,收盘价,最高价,最低价,开盘价,前收盘,涨跌额,换手率,成交量,成交金额,总市值,流通市值\r"; // 缺少\n
		DayLine.SetStockCode("600000.SS");
		EXPECT_FALSE(DayLine.SkipNeteaseDayLineInformationHeader(str, lCurrentPos));
		EXPECT_EQ(lCurrentPos, str.size());
		lCurrentPos = 0;

		str = "日期,股票代码,名称,收盘价,最高价,最低价,开盘价,前收盘,涨跌额,换手率,成交量,成交金额,总市值,流通市值"; // 缺少\r
		DayLine.SetStockCode("600000.SS");
		EXPECT_FALSE(DayLine.SkipNeteaseDayLineInformationHeader(str, lCurrentPos));
		EXPECT_EQ(lCurrentPos, str.size());
		lCurrentPos = 0;
