...
void CWebRTData::ParseSinaData(const string_view& svData) {
	...
	m_time = ConvertBufferToTime("%04d-%02d-%02d %02d:%02d:%02d", sTime.c_str(), gl_pChinaMarket->GetMarketTimeZone());	//转成UTC时间。新浪实时数据的时区为东八区
...
void CWebRTData::ParseTengxunData(const string_view& svData) {
	...
	m_time = ConvertBufferToTime("%04d%02d%02d%02d%02d%02d", sTime.c_str(), gl_pChinaMarket->GetMarketTimeZone()); // 转成UTC时间。腾讯实时数据的时区为东八区
	// 涨跌
	sv = GetNextField(svData, lCurrentPos, '~'); //
	// 涨跌率
	sv = GetNextField(svData, lCurrentPos, '~'); //
