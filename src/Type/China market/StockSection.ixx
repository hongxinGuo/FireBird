module;

export module StockSection;

import std;
using std::string;
using std::shared_ptr;

export {
	class CStockSection {
	public:
		CStockSection();
		virtual ~CStockSection() = default;

		bool IsActive() const noexcept { return m_fActive; }
		void SetActive(const bool fFlag) noexcept { m_fActive = fFlag; }
		int GetIndexNumber() const noexcept { return m_lIndexNumber; }
		void SetIndexNumber(const int lNumber) noexcept { m_lIndexNumber = lNumber; }
		int GetMarket() const noexcept { return m_wMarket; }
		void SetMarket(const int wMarket) noexcept { m_wMarket = wMarket; }
		string GetComment() const { return m_strComment; }
		void SetComment(const string& strComment) { m_strComment = strComment; }
		bool IsBuildStockPtr() const noexcept { return m_fBuildStockPtr; }
		void SetBuildStockPtr(const bool fFlag) noexcept { m_fBuildStockPtr = fFlag; }

	protected:
		// 需存储数据
		bool m_fActive; // 本段代码是否已经使用
		int m_lIndexNumber; // 本段代码3个高位的数字
		int m_wMarket; // 市场
		string m_strComment; // 注释

		// 无需存储数据
		bool m_fBuildStockPtr; // 本段证券是否已建立证券指针（在m_vChinaMarketStock中）。
	};

	using CStockSectionPtr = shared_ptr<CStockSection>;
}