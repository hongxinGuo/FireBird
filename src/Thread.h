...
UINT ThreadBuildDayLineRS(not_null<CChinaMarket*> pMarket, long startCalculatingDate); // 此工作线程返回值为11, 参数为当前最后计算日期
...
// 装载临时存储的实时数据线程
UINT ThreadLoadTempRTData(not_null<CChinaMarket*> pMarket, long lTheDate); // 此线程返回值为19
// 计算和存储当前交易日数据线程
UINT ThreadProcessTodayStock(not_null<CChinaMarket*> pMarket); // 此工作线程返回值为14
// 存储一个股票的日线历史数据。
UINT ThreadSaveDayLineBasicInfo(not_null<CChinaStock*> pStock); // 此工作线程返回值为15, 参数为携带智能指针的一个结构指针
// 从数据库中读取日线历史数据线程
UINT ThreadLoadDayLine(not_null<CChinaStock*> pStockInput); // 此工作线程返回值为16
// 维护日线数据库线程（尚未实现）
...
// 更新选项数据库
UINT ThreadAppendChosenStockDB(CChinaMarket* pMarket); // 此线程返回值为22
// 生成周线历史数据
UINT ThreadBuildWeekLine(not_null<CChinaMarket*> pMarket, long lStartDate); // 此线程的返回值为25
// 生成股票的周线历史数据
UINT ThreadBuildWeekLineOfStock(not_null<CChinaStock*> pStockInput, long lStartDate); /// 此线程的返回值为26
// 从数据库读取周线历史数据
UINT ThreadLoadWeekLine(not_null<CChinaStock*> pStockInput); // 此线程返回值为29
...
// 存储证券代码段索引
UINT ThreadSaveStockSection(not_null<CChinaMarket*> pMarket); // 此工作线程返回值为35

// 下面为美国股票市场的工作线程
// 更新美股日线起止日期线程
UINT ThreadUpdateWorldMarketStockProfileDB(CWorldMarket* pMarket); // 此工作线程返回值为37
// 更新Forex日线数据
...
// 更新EPSSurprise表
UINT ThreadUpdateEPSSurpriseDB(not_null<CWorldStock*> pStockInput); // 此工作线程返回值为41
// 更新美国股票的日线历史数据。
...
// 更新Tiingo股票集。
UINT ThreadUpdateTiingoStockDB(not_null<CWorldMarket*> pMarket); // 此工作线程返回值为44
// 更新Tiingo工业分类。
UINT ThreadUpdateTiingoIndustry(not_null<CWorldMarket*> pMarket); // 此工作线程返回值为45
// 更新SIC工业分类。
UINT ThreadUpdateSICIndustry(not_null<CWorldMarket*> pMarket); // 此工作线程返回值为46
// 更新Naics工业分类。
UINT ThreadUpdateNaicsIndustry(not_null<CWorldMarket*> pMarket); // 此工作线程返回值为47
...
// 更新Crypto Exchange
UINT ThreadUpdateCryptoExchangeDB(not_null<CWorldMarket*> pMarket); // 此工作线程返回值为51
// 更新Crypto代码表
UINT ThreadUpdateFinnhubCryptoSymbolDB(not_null<CWorldMarket*> pMarket); // 此工作线程返回值为52
// 更新Crypto日线数据
...
UINT ThreadDistributeAndCalculateRTData(not_null<CChinaMarket*> pMarket); // 此线程返回值为108
