...
bool CProductTiingoStockSymbol::ParseAndStoreWebData(CWebDataPtr pWebData) {
	...
	if (!pvTiingoStock->empty()) {
		...
		for (const auto& pTiingoStock : *pvTiingoStock) {
			if (!dynamic_pointer_cast<CWorldMarket>(GetMarket())->IsTiingoStock(pTiingoStock->m_strTicker)) { dynamic_pointer_cast<CWorldMarket>(GetMarket())->AddTiingoStock(pTiingoStock); }
			if (dynamic_pointer_cast<CWorldMarket>(GetMarket())->IsStock(pTiingoStock->m_strTicker)) {
				// Tiingo的Symbol信息只是用于Finnhub的一个补充。
				lTemp++;
				const auto pStock = dynamic_pointer_cast<CWorldMarket>(GetMarket())->GetStock(pTiingoStock->m_strTicker);
