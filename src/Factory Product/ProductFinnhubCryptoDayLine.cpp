...
CString CProductFinnhubCryptoDayLine::CreateMessage() {
	...
	const auto pCryptoSymbol = dynamic_pointer_cast<CWorldMarket>(m_pMarket)->GetFinnhubCryptoSymbol(m_lIndex);
...
bool CProductFinnhubCryptoDayLine::ParseAndStoreWebData(CWebDataPtr pWebData) {
	...
	if (!pvDayLine->empty()) {
		for (const auto& pDayLine : *pvDayLine) {
			...
			const long lTemp = ConvertToDate(pDayLine->m_time, dynamic_pointer_cast<CWorldMarket>(m_pMarket)->GetMarketTimeZone());
