...
void CProductFinnhub::AddInaccessibleSymbol() {
	...
	try { // 存在此申请类型
		...
		if (!pExchange->HaveSymbol(m_strInquiringExchange.GetString())) {	// 新的交易所代码？
			pExchange->AddSymbol(m_strInquiringExchange.GetString());
		}
		...
		pNewExchange->AddSymbol(m_strInquiringExchange.GetString());
