...
CString CProductFinnhubForexSymbol::CreateMessage() {
	...
	const auto strParam = dynamic_pointer_cast<CWorldMarket>(m_pMarket)->GetForexExchange(m_lIndex);
...
bool CProductFinnhubForexSymbol::ParseAndStoreWebData(CWebDataPtr pWebData) {
	...
	for (const auto& pSymbol : *pvForexSymbol) {
		if (!dynamic_pointer_cast<CWorldMarket>(m_pMarket)->IsForexSymbol(pSymbol->GetSymbol())) {
			pSymbol->SetExchangeCode(dynamic_pointer_cast<CWorldMarket>(m_pMarket)->GetForexExchange(m_lIndex));
			dynamic_pointer_cast<CWorldMarket>(m_pMarket)->AddForexSymbol(pSymbol);
		}
	}
