...
[[nodiscard]] long GetNextSecond(const long lTime); // 得到lTime的下一秒时间（HHMMSS制式）
[[nodiscard]] long GetNextTime(const long lTime, long hh, long mm, long ss);
...
[[nodiscard]] CString ConvertDateToString(const long lDate, const CString& strFormat);
...
[[nodiscard]] time_t ConvertBufferToTime(CString strFormat, const char* BufferMarketTime, const time_t tTimeZoneOffset = -8 * 3600); // 默认采用东八区标准时间
