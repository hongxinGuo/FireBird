...
void SaveToFile(const string& strFileName, const string& s) {
	CFile f;
	if (f.Open(Utf8ToW(strFileName).c_str(), CFile::modeCreate | CFile::modeWrite | CFile::typeBinary)) {
...
bool LoadFromFile(const string& strFileName, string& s) {
	char buffer[1025]{};
	int iRead;
	CFile f(Utf8ToW(strFileName).c_str(), CFile::modeRead | CFile::typeBinary);
	int i = 0;
	do {
