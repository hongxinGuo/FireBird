...
void COutputWnd::AdjustHorzScroll(CListBox& wndListBox) {
	...
	for (int i = 0; i < wndListBox.GetCount(); i++) {
		...
		cxExtentMax = max(cxExtentMax, static_cast<int>(dc.GetTextExtent(strItem).cx));
	}
...
void COutputWnd::OnTimer(UINT_PTR nIDEvent) {
	...
	if (time > m_timeLast) {// 每秒更新一次
		...
		for (const auto& pTask : validChinaTasks) {
			string s = std::format("{}: {:06Ld}: {}", sTime, toFormattedTime(pTask->GetTime()), gl_mapMarketMapIndex.at(pTask->GetType()));
