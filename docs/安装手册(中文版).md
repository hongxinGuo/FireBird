...
1.1 SQL数据库用于存储系统接收到的数据和系统的测试数据。本系统使用社区版的MySQL数据库，版本为8.4.X。

1.1.1 安装MySQL社区版（位于\Lib\MySQL目录中），RAM4G.ini和RAM16G.ini分别是4G内存和16G内存的默认配置文件。

1.1.2 在"控制面板"的"Windows工具"中，选择"ODBC数据源(64位)",添加四个MySQL ODBC unicode driver,默认名称分别为、stock_market和stock_market_test，然后分别配置其相应的用户名和密码。
chinaMarket和stock_market的默认账户和密码为：FireBird firebird，stock_market_test的默认账户和密码为：Test test。
...
###2. 系统使用vcpkg加载第三方库，包括nlohmann-json、ms-gsl、zlib。
      concurrencpp、concurrentqueue、openssl、、googleTest、google Banchmark、simdjson直接使用包含文件或者编译后的库文件。
