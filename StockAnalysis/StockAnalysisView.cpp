...
void CStockAnalysisView::ShowBuySell(CDC* pDC, CChinaStockPtr pStock, CRect rectArea) {
  ...
  rectTop.top = yBase * (1 - (double)pStock->GetUnknownVolume() * dRatio / base);
...
void CStockAnalysisView::ShowOrdinaryBuySell(CDC* pDC, CChinaStockPtr pStock, CRect rectArea) {
  ...
  rectTop.left = rectArea.left;
  rectTop.right = rectArea.left + 30;
  rectTop.top = yBase * (1 - (double)pStock->GetOrdinaryBuyVolumeBelow5000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 31;
  rectTop.right = rectArea.left + 60;
  rectTop.top = yBase * (1 - (double)pStock->GetOrdinaryBuyVolumeBelow10000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 61;
  rectTop.right = rectArea.left + 90;
  rectTop.top = yBase * (1 - (double)pStock->GetOrdinaryBuyVolumeBelow20000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 91;
  rectTop.right = rectArea.left + 120;
  rectTop.top = yBase * (1 - (double)pStock->GetOrdinaryBuyVolumeBelow50000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 121;
  rectTop.right = rectArea.left + 150;
  rectTop.top = yBase * (1 - (double)pStock->GetOrdinaryBuyVolumeBelow100000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 151;
  rectTop.right = rectArea.left + 180;
  rectTop.top = yBase * (1 - (double)pStock->GetOrdinaryBuyVolumeBelow200000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 181;
  rectTop.right = rectArea.left + 210;
  rectTop.top = yBase * (1 - (double)pStock->GetOrdinaryBuyVolumeAbove200000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);

  rectBottom.top = yBase;
  rectBottom.left = rectArea.left;
  rectBottom.right = rectArea.left + 30;
  rectBottom.bottom = yBase * (1 + (double)pStock->GetOrdinarySellVolumeBelow5000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 31;
  rectBottom.right = rectArea.left + 60;
  rectBottom.bottom = yBase * (1 + (double)pStock->GetOrdinarySellVolumeBelow10000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 61;
  rectBottom.right = rectArea.left + 90;
  rectBottom.bottom = yBase * (1 + (double)pStock->GetOrdinarySellVolumeBelow20000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 91;
  rectBottom.right = rectArea.left + 120;
  rectBottom.bottom = yBase * (1 + (double)pStock->GetOrdinarySellVolumeBelow50000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 121;
  rectBottom.right = rectArea.left + 150;
  rectBottom.bottom = yBase * (1 + (double)pStock->GetOrdinarySellVolumeBelow100000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 151;
  rectBottom.right = rectArea.left + 180;
  rectBottom.bottom = yBase * (1 + (double)pStock->GetOrdinarySellVolumeBelow200000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 181;
  rectBottom.right = rectArea.left + 210;
  rectBottom.bottom = yBase * (1 + (double)pStock->GetOrdinarySellVolumeAbove200000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
}
...
void CStockAnalysisView::ShowCanceledBuySell(CDC* pDC, CChinaStockPtr pStock, CRect rectArea) {
  ...
  rectTop.left = rectArea.left;
  rectTop.right = rectArea.left + 30;
  rectTop.top = yBase * (1 - (double)pStock->GetCanceledBuyVolumeBelow5000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 31;
  rectTop.right = rectArea.left + 60;
  rectTop.top = yBase * (1 - (double)pStock->GetCanceledBuyVolumeBelow10000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 61;
  rectTop.right = rectArea.left + 90;
  rectTop.top = yBase * (1 - (double)pStock->GetCanceledBuyVolumeBelow20000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 91;
  rectTop.right = rectArea.left + 120;
  rectTop.top = yBase * (1 - (double)pStock->GetCanceledBuyVolumeBelow50000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 121;
  rectTop.right = rectArea.left + 150;
  rectTop.top = yBase * (1 - (double)pStock->GetCanceledBuyVolumeBelow100000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 151;
  rectTop.right = rectArea.left + 180;
  rectTop.top = yBase * (1 - (double)pStock->GetCanceledBuyVolumeBelow200000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 181;
  rectTop.right = rectArea.left + 210;
  rectTop.top = yBase * (1 - pStock->GetCanceledBuyVolumeAbove200000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);

  rectBottom.top = yBase;
  rectBottom.left = rectArea.left;
  rectBottom.right = rectArea.left + 30;
  rectBottom.bottom = yBase * (1 + pStock->GetCanceledSellVolumeBelow5000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 31;
  rectBottom.right = rectArea.left + 60;
  rectBottom.bottom = yBase * (1 + pStock->GetCanceledSellVolumeBelow10000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 61;
  rectBottom.right = rectArea.left + 90;
  rectBottom.bottom = yBase * (1 + pStock->GetCanceledSellVolumeBelow20000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 91;
  rectBottom.right = rectArea.left + 120;
  rectBottom.bottom = yBase * (1 + pStock->GetCanceledSellVolumeBelow50000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 121;
  rectBottom.right = rectArea.left + 150;
  rectBottom.bottom = yBase * (1 + pStock->GetCanceledSellVolumeBelow100000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 151;
  rectBottom.right = rectArea.left + 180;
  rectBottom.bottom = yBase * (1 + pStock->GetCanceledSellVolumeBelow200000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 181;
  rectBottom.right = rectArea.left + 210;
  rectBottom.bottom = yBase * (1 + pStock->GetCanceledSellVolumeAbove200000() * dRatio / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
}
