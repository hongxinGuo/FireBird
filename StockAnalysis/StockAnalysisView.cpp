...
void CStockAnalysisView::ShowOrdinaryBuySell(CDC* pDC, CChinaStockPtr pStock, CRect rectArea) {
  ...
  rectTop.top = yBase * (1 - pStock->GetOrdinaryBuyVolumeBelow5000() * 100.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 31;
  rectTop.right = rectArea.left + 60;
  rectTop.top = yBase * (1 - pStock->GetOrdinaryBuyVolumeBelow10000() * 50.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 61;
  rectTop.right = rectArea.left + 90;
  rectTop.top = yBase * (1 - pStock->GetOrdinaryBuyVolumeBelow20000() * 20.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 91;
  rectTop.right = rectArea.left + 120;
  rectTop.top = yBase * (1 - pStock->GetOrdinaryBuyVolumeBelow50000() * 10.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 121;
  rectTop.right = rectArea.left + 150;
  rectTop.top = yBase * (1 - pStock->GetOrdinaryBuyVolumeBelow100000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 151;
  rectTop.right = rectArea.left + 180;
  rectTop.top = yBase * (1 - pStock->GetOrdinaryBuyVolumeBelow200000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 211;
  rectTop.right = rectArea.left + 240;
  rectTop.top = yBase * (1 - pStock->GetOrdinaryBuyVolumeAbove200000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);

  rectBottom.top = yBase;
  rectBottom.left = rectArea.left;
  rectBottom.right = rectArea.left + 30;
  rectBottom.bottom = yBase * (1 + pStock->GetOrdinarySellVolumeBelow5000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 31;
  rectBottom.right = rectArea.left + 60;
  rectBottom.bottom = yBase * (1 + pStock->GetOrdinarySellVolumeBelow10000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 61;
  rectBottom.right = rectArea.left + 90;
  rectBottom.bottom = yBase * (1 + pStock->GetOrdinarySellVolumeBelow20000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 91;
  rectBottom.right = rectArea.left + 120;
  rectBottom.bottom = yBase * (1 + pStock->GetOrdinarySellVolumeBelow50000() * 10.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 121;
  rectBottom.right = rectArea.left + 150;
  rectBottom.bottom = yBase * (1 + pStock->GetOrdinarySellVolumeBelow100000() * 20.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 151;
  rectBottom.right = rectArea.left + 180;
  rectBottom.bottom = yBase * (1 + pStock->GetOrdinarySellVolumeBelow200000() * 50.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 211;
  rectBottom.right = rectArea.left + 240;
  rectBottom.bottom = yBase * (1 + pStock->GetOrdinarySellVolumeAbove200000() * 100.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
}
...
void CStockAnalysisView::ShowCanceledBuySell(CDC* pDC, CChinaStockPtr pStock, CRect rectArea) {
  ...
  rectTop.left = rectArea.left;
  rectTop.right = rectArea.left + 30;
  rectTop.top = yBase * (1 - pStock->GetCancelBuyVolumeBelow5000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 31;
  rectTop.right = rectArea.left + 60;
  rectTop.top = yBase * (1 - pStock->GetCancelBuyVolumeBelow10000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 61;
  rectTop.right = rectArea.left + 90;
  rectTop.top = yBase * (1 - pStock->GetCancelBuyVolumeBelow20000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 91;
  rectTop.right = rectArea.left + 120;
  rectTop.top = yBase * (1 - pStock->GetCancelBuyVolumeBelow50000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 121;
  rectTop.right = rectArea.left + 150;
  rectTop.top = yBase * (1 - pStock->GetCancelBuyVolumeBelow100000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 151;
  rectTop.right = rectArea.left + 180;
  rectTop.top = yBase * (1 - pStock->GetCancelBuyVolumeBelow200000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);
  rectTop.left = rectArea.left + 211;
  rectTop.right = rectArea.left + 240;
  rectTop.top = yBase * (1 - pStock->GetCancelBuyVolumeAbove200000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectTop, crRed);

  rectBottom.top = yBase;
  rectBottom.left = rectArea.left;
  rectBottom.right = rectArea.left + 30;
  rectBottom.bottom = yBase * (1 + pStock->GetCancelSellVolumeBelow5000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 31;
  rectBottom.right = rectArea.left + 60;
  rectBottom.bottom = yBase * (1 + pStock->GetCancelSellVolumeBelow10000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 61;
  rectBottom.right = rectArea.left + 90;
  rectBottom.bottom = yBase * (1 + pStock->GetCancelSellVolumeBelow20000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 91;
  rectBottom.right = rectArea.left + 120;
  rectBottom.bottom = yBase * (1 + pStock->GetCancelSellVolumeBelow50000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 121;
  rectBottom.right = rectArea.left + 150;
  rectBottom.bottom = yBase * (1 + pStock->GetCancelSellVolumeBelow100000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 151;
  rectBottom.right = rectArea.left + 180;
  rectBottom.bottom = yBase * (1 + pStock->GetCancelSellVolumeBelow200000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
  rectBottom.left = rectArea.left + 211;
  rectBottom.right = rectArea.left + 240;
  rectBottom.bottom = yBase * (1 + pStock->GetCancelSellVolumeAbove200000() * 2.0 / base);
  SysCallFillSolidRect(pDC, rectBottom, crGreen);
}
