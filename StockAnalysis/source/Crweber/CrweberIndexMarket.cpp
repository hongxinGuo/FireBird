...
bool CCrweberIndexMarket::TaskProcessWebRTDataGetFromCrweberdotcom(void) {
	...
	const size_t lTotalData = gl_WebInquirer.GetCrweberDataSize();
	for (int i = 0; i < lTotalData; i++) {
		...
		if (m_CrweberIndex.ReadData(pWebData)) {
			...
			if (!m_fTodayDataUpdated || m_CrweberIndex.IsDataChanged(m_CrweberIndexLast)) {
				...
				CreatingThreadSaveCrweberDB(pCrweberIndex);
...
bool CCrweberIndexMarket::CreatingThreadMaintainDB(void) {
...
bool CCrweberIndexMarket::CreatingThreadSaveCrweberDB(CCrweberIndexPtr pCrweberIndex) {
