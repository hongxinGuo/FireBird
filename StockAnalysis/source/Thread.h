...
UINT ThreadReadNeteaseDL(CNeteaseDLWebInquiry* pNeteaseDLWebData);      // 此工作线程返回值为4，参数为pNeteaseWebDLData
...
UINT ThreadBuildDLRS(CChinaMarket* pMarket, long startCalculatingDay); // 此工作线程返回值为11, 参数为当前最后计算日期
// 计算给定日期股票日线相对强度线程。此线程由ThreadCalculateRS线程调用，用于并发计算不同日期的日线相对强度，以提高效率
UINT ThreadBuildDLRSOfDate(CChinaMarket* pMarket, long lDate); // 此工作线程返回值也为12
// 存储临时系统状态线程
UINT ThreadSaveTempRTData(CChinaMarket* pMarket);     // 此工作线程返回值为13
// 计算和存储当前交易日数据线程
UINT ThreadProcessTodayStock(CChinaMarket* pMarket);     // 此工作线程返回值为14
// 存储一个股票的日线历史数据。
UINT ThreadSaveDLBasicInfoOfStock(CChinaStockPtr pStock); // 此工作线程返回值为15, 参数为携带智能指针的一个结构指针
// 从数据库中读取日线历史数据线程
UINT ThreadLoadDL(CChinaStockPtr pStock);        // 此工作线程返回值为16
// 维护日线数据库线程（尚未实现）
UINT ThreadMaintainDLDataBase(void);   // 此工作线程返回值为17
//更新股票代码数据库线程
...
UINT ThreadBuildWeekLineRS(CChinaMarket* pMarket, long startCalculatingDate); // 此工作线程返回值为30, 参数为当前最后计算日期
// 计算给定日期股票日线相对强度线程。此线程由ThreadCalculateRS线程调用，用于并发计算不同日期的日线相对强度，以提高效率
UINT ThreadBuildWeekLineRSOfDate(CChinaMarket* pMarket, long lDate); // 此工作线程返回值也为31
// 计算本周的周线
