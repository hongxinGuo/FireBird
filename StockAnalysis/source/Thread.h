...
// 存储一个股票的日线历史数据。
UINT ThreadSaveDayLineBasicInfoOfStock(not_null<CChinaStock*> pStock); // 此工作线程返回值为15, 参数为携带智能指针的一个结构指针
// 从数据库中读取日线历史数据线程
UINT ThreadLoadDayLine(not_null<CChinaStock*> pStock);        // 此工作线程返回值为16
// 维护日线数据库线程（尚未实现）
...
// 生成股票的周线历史数据
UINT ThreadBuildWeekLineOfStock(not_null<CChinaStock*> pStock, long lStartDate); /// 此线程的返回值为26
// 从数据库读取周线历史数据
UINT ThreadLoadWeekLine(not_null<CChinaStock*> pStock); // 此线程返回值为29
...
// 下面为美国股票市场的工作线程
// 更新美股日线起止日期线程
UINT ThreadUpdateStockProfileDB(not_null<CWorldMarket*> pMarket); // 此工作线程返回值为37
// 更新Forex日线数据
...
// 更新EPSSurprise表
UINT ThreadUpdateEPSSurpriseDB(not_null<CWorldStock*> pStock); // 此工作线程返回值为41
// 更新美国股票的日线历史数据。
...
UINT ThreadUpdateBasicFinancialAnnualDB(not_null<CWorldStock*> pStock); // 此工作线程返回值为56
// 更新Finnhub basic financial quarterly data
UINT ThreadUpdateBasicFinancialQuarterlyDB(not_null<CWorldStock*> pStock); // 此工作线程返回值为57
