...
// 存储一个股票的日线历史数据。
UINT ThreadSaveDayLineBasicInfoOfStock(not_null<CChinaStockPtr> pStock); // 此工作线程返回值为15, 参数为携带智能指针的一个结构指针
// 从数据库中读取日线历史数据线程
UINT ThreadLoadDayLine(not_null<CChinaStockPtr> pStock);        // 此工作线程返回值为16
// 维护日线数据库线程（尚未实现）
...
// 生成股票的周线历史数据
UINT ThreadBuildWeekLineOfStock(not_null<CChinaStockPtr> pStock, long lStartDate); /// 此线程的返回值为26
// 从数据库读取周线历史数据
UINT ThreadLoadWeekLine(not_null<CChinaStockPtr> pStock); // 此线程返回值为29
...
// 存储证券代码段索引
UINT ThreadSaveSectionIndex(not_null<CChinaMarket*> pMarket); // 此工作线程返回值为35
