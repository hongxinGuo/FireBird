...
// 存储一个股票的日线历史数据。
UINT ThreadSaveDayLineBasicInfoOfStock(not_null<CChinaStake*> pStake); // 此工作线程返回值为15, 参数为携带智能指针的一个结构指针
// 从数据库中读取日线历史数据线程
UINT ThreadLoadDayLine(not_null<CChinaStake*> pStake);        // 此工作线程返回值为16
// 维护日线数据库线程（尚未实现）
...
// 生成股票的周线历史数据
UINT ThreadBuildWeekLineOfStock(not_null<CChinaStake*> pStake, long lStartDate); /// 此线程的返回值为26
// 从数据库读取周线历史数据
UINT ThreadLoadWeekLine(not_null<CChinaStake*> pStake); // 此线程返回值为29
...
// 下面为美国股票市场的工作线程
// 更新一个美国股票的日线历史数据。
UINT ThreadUpdateAmericaStakeDayLineDB(not_null<CAmericaStake*> pStake); // 此工作线程返回值为36, 参数为携带智能指针的一个结构指针
// 更新美股日线起止日期线程
...
// 更新Forex代码表
UINT ThreadUpdateForexSymbolDB(not_null<CAmericaMarket*> pMarket); // 此工作线程返回值为39
// 更新CountryList代码表
UINT ThreadUpdateCountryListDB(not_null<CAmericaMarket*> pMarket); // 此工作线程返回值为40
// 更新EPSSurprise表
