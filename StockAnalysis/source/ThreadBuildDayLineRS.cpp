...
//
// 此线程调用ThreadBuildDLRSOfDay线程，目前最多允许同时生成8个线程。
//
//
///////////////////////////////////////////////////////////////////////////////////
#include"globedef.h"
...
UINT ThreadBuildDLRS(CChinaMarket* pMarket, long startCalculatingDate) {
  ...
  do {
    if (pMarket->IsWorkingDay(ctCurrent)) { // 星期六和星期日无交易，略过
      ...
      pMarket->RunningThreadBuildDLRSOfDay(lToday);
    }
    ctCurrent += oneDay;
...
//
/////////////////////////////////////////////////////////////////////////////////////////
UINT ThreadBuildDLRSOfDay(CChinaMarket* pMarket, long lDate) {
  gl_ThreadStatus.IncreaseRunningThread();
  ...
  if (!gl_fExitingSystem && !gl_fExitingCalculatingRS) {
    pMarket->BuildDLRSOfDay(lDate);  // 调用实际执行函数
  }
