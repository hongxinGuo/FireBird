...
UINT ThreadReadNeteaseDayLine(not_null<CNeteaseDayLineWebInquiry*> pNeteaseDayLineWebData) {
  ...
  if (pNeteaseDayLineWebData->ReadWebData3(/*siDelayTime*/ 500, 50, 30)) { // 这种是采用等待时间的方式，目前可行。
    // 将读取的日线数据放入相关股票的日线数据缓冲区中，并设置相关标识。
    pStake = gl_pChinaStockMarket->GetStock(pNeteaseDayLineWebData->GetDownLoadingStockCode());
