// SetWeekLineBasicInfo.h : CSetWeekLineBasicInfo 类的实现

// CSetWeekLineBasicInfo 实现
...
IMPLEMENT_DYNAMIC(CSetWeekLineBasicInfo, CRecordset)

CSetWeekLineBasicInfo::CSetWeekLineBasicInfo(CDatabase* pdb)
  : CRecordset(pdb) {
  m_ID = 0;
  m_Date = 0;
  m_Market = 0;
...
CString CSetWeekLineBasicInfo::GetDefaultConnect() {
  return GetChinaMarketSchemaConnect();
}

CString CSetWeekLineBasicInfo::GetDefaultSQL() {
  return _T("[WeekLine]");
}

void CSetWeekLineBasicInfo::DoFieldExchange(CFieldExchange* pFX) {
...
#ifdef _DEBUG
void CSetWeekLineBasicInfo::AssertValid() const {
  CRecordset::AssertValid();
}

void CSetWeekLineBasicInfo::Dump(CDumpContext& dc) const {
  CRecordset::Dump(dc);
}
#endif //_DEBUG