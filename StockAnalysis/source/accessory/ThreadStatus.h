...
public:
	...
	bool IsBackGroundThreadsWorking(void) const noexcept { if (m_NumberOfBackGroundWorkingThreads > 0) return true; else return false; } //计算日线的线程是否处于运行中
	...
	int GetNumberOfSavingThread(void) const noexcept { const int iNumber = m_NumberOfSavingThread; return iNumber; }
	bool IsSavingThreadRunning(void) const noexcept { if (GetNumberOfSavingThread() > 0) return true; else return false; }
	...
	int GetNumberOfWebInquiringThread(void) const noexcept { const int iNumber = m_NumberOfWebInquiringThread; return iNumber; }
	bool IsWebInquiringThreadRunning(void) const noexcept { if (GetNumberOfWebInquiringThread() > 0) return true; else return false; }
