...
bool CFinnhubDataSource::InquireCompanyBasicFinancial(void) {
	...
	ASSERT(gl_pWorldMarket->IsSystemReady());
	if (!IsStockBasicFinancialUpdated()) {
...
bool CFinnhubDataSource::InquireInsiderTransaction(void) {
	...
	ASSERT(gl_pWorldMarket->IsSystemReady());
	if (!IsInsiderTransactionUpdated()) {
...
bool CFinnhubDataSource::InquireEPSSurprise(void) {
	...
	if (!IsEPSSurpriseUpdated()) {
		...
		if (fFound) {
			fHaveInquiry = true;
			CProductWebSourceDataPtr product = m_FinnhubFactory.CreateProduct(gl_pWorldMarket.get(), iInquiryType);
...
bool CFinnhubDataSource::InquireCryptoDayLine(void) {
	...
	ASSERT(gl_pWorldMarket->IsSystemReady());
	ASSERT(!IsInquiring()); //调用此函数时，必须确保没有查询任务在执行。
	if (!IsCryptoDayLineUpdated()) {
