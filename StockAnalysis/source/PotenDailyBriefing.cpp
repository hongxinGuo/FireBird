...
bool CPotenDailyBriefing::ReadData(CWebDataReceivedPtr pWebDataReceived) {
  ...
  while (pWebDataReceived->GetCurrentPos() < pWebDataReceived->GetBufferLength()) {
    ...
    if (strHead.Compare(_T("DIRTY TANKER")) == 0) {
      for (int i = 0; i < 5; i++) strNoUse = GetNextString(pWebDataReceived); // 抛掉5个没用字符串
      str1 = GetNextString(pWebDataReceived); // "VLCC"
      m_dTD3C = GetOneValue(str1);

      for (int i = 0; i < 3; i++) strNoUse = GetNextString(pWebDataReceived); // 抛掉5个没用字符串
      ...
      for (int i = 0; i < 9; i++) strNoUse = GetNextString(pWebDataReceived); // 抛掉9个没用字符串
      ...
      if (strHead.Compare(_T("TIME CHARTER")) == 0) {
        ...
        for (int i = 0; i < 8; i++) strNoUse = GetNextString(pWebDataReceived); // 抛掉4个没用字符串
        ...
        for (int i = 0; i < 2; i++) strNoUse = GetNextString(pWebDataReceived); // 抛掉4个没用字符串
        str1 = GetNextString(pWebDataReceived);
